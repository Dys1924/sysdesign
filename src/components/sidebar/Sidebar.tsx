...
import { CATEGORY_STYLE, type NodeCategory, type NodeTemplate } from '../../types/diagram'
...
const CATEGORY_ORDER: NodeCategory[] = ['microservice', 'cloud', 'database', 'frontend']

function NodeItem({ template, isCustom = false }: { template: NodeTemplate, isCustom?: boolean }) {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: `color-mix(in srgb, ${style.color} 15%, var(--card))` }}
      >
...
export default function Sidebar() {
  ...
  const grouped = React.useMemo(() => {
    const items: Record<NodeCategory, NodeTemplate[]> = {
      microservice: [], cloud: [], database: [], frontend: []
    }
    ...
    <aside className="relative h-full w-60 flex flex-col overflow-hidden rounded-xl border border-border bg-card/95 backdrop-blur-md shadow-2xl z-20">
      <div className="shrink-0 px-3.5 py-3 border-b border-border bg-transparent z-10 transition-colors">
        ...
        <div className="relative">
          <TablerIcons.IconSearch className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" size={14} />
          <input
            type="text"
            ...
            return (
              <div key={cat} className="mb-1">
                ...
                {isExpanded && (
                  <div className="py-1">
                    {items.map(t => (
                      <NodeItem key={t.subtype} template={t} />
                    ))}
                  </div>
                )}
              </div>
      ...
      <div className="shrink-0 mt-auto border-t border-border bg-card">
        ...
        <div className="px-3.5 py-3 border-t border-border text-[10px] text-muted-foreground leading-relaxed">
          Drag onto canvas to add.<br />
